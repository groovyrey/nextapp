  ...
  if (isUid) {
    try {
      ...
      if (userDoc.exists) {
        ...
        // Convert Firestore Timestamp objects to plain JavaScript Date objects or strings
        if (authorDetails && authorDetails.lastUsernameChange && typeof authorDetails.lastUsernameChange.toDate === 'function') {
          authorDetails.lastUsernameChange = authorDetails.lastUsernameChange.toDate().toISOString();
        }
      } else {
